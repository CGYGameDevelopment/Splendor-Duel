...
function updatePlayer(state, id, playerStateUpdate) {
    const players = [{ ...state.players[0] }, { ...state.players[1] }];
    players[id] = { ...players[id], ...playerStateUpdate };
    return { ...state, players };
}
// ─── Replenish board from bag ─────────────────────────────────────────────────
function replenishBoard(state) {
    let bag = { ...state.bag };
    const board = [...state.board];
    for (const index of board_1.SPIRAL_ORDER) {
        if (board[index] !== null)
            continue; // already occupied
        // Find a color available in bag
        const available = helpers_1.TOKEN_COLORS.filter(c => bag[c] > 0);
...
function refillPyramidSlot(state, level, removedCardId) {
...
// ─── Pyramid card lookup ──────────────────────────────────────────────────────
function findCardInPyramid(pyramid, cardId) {
    for (const lvl of helpers_1.CARD_LEVELS) {
        const key = `level${lvl}`;
        const found = pyramid[key].find(c => c.id === cardId);
        if (found)
            return { card: found, level: lvl };
    }
    return null;
}
function locateCardForPurchase(pyramid, reservedCards, cardId) {
    const pyramidResult = findCardInPyramid(pyramid, cardId);
    if (pyramidResult)
        return { card: pyramidResult.card, fromReserve: false, level: pyramidResult.level };
    const found = reservedCards.find(c => c.id === cardId);
    if (found)
        return { card: found, fromReserve: true };
    return null;
}
// ─── Token cost deduction ─────────────────────────────────────────────────────
function deductTokenCost(playerTokens, bag, cost, goldUsage) {
...
function applyPurchase(state, cp, purchasedCard, fromReserve, level, goldUsage) {
    const player = state.players[cp];
    const cost = (0, helpers_1.netCost)(purchasedCard, player);
...
function postPurchaseTransition(state, milestoneCrossed) {
...
function endOfTurnSequence(state) {
    const cp = state.currentPlayer;
    const player = state.players[cp];
    // 4.1 Victory Check
    const victoryCondition = (0, helpers_1.checkVictory)(player);
    if (victoryCondition) {
        return {
            ...state,
            phase: 'game_over',
    ...
    // 4.2 Discard Check
    if ((0, helpers_1.totalTokens)(player.tokens) > helpers_1.MAX_TOKENS) {
...
function resolveAbility(state, card) {
    if (!card.ability)
        return state;
    switch (card.ability) {
        case 'Turn': {
        ...
        case 'Privilege': {
            const { privileges, players } = (0, helpers_1.grantPrivileges)(state, state.currentPlayer, 1);
        ...
        case 'Take': {
            ...
            const hasEligible = helpers_1.GEM_COLORS.some(c => oppTokens[c] > 0) || oppTokens.pearl > 0;
            if (!hasEligible)
                return { ...state, pendingAbility: null };
        ...
        case 'Wild/Turn': {
            ...
            const hasColoredCard = player.purchasedCards.some(c => c.id !== card.id && c.color !== 'wild' && c.color !== null);
            if (!hasColoredCard) {
                return { ...state, pendingAbility: null };
            }
    ...
    switch (action.type) {
        case 'END_OPTIONAL_PHASE': {
            if (state.phase === 'optional_privilege')
                return { ...state, phase: 'optional_replenish' };
            if (state.phase === 'optional_replenish')
        ...
        case 'USE_PRIVILEGE': {
            ...
            if (privilegesUsed === 0 || privilegesUsed > player.privileges)
                return state;
            ...
            for (const idx of indices) {
                ...
                if (!cell || cell === 'gold')
                    return state; // cell must have a non-gold token
                board[idx] = null;
        ...
        case 'REPLENISH_BOARD': {
            if (state.phase !== 'optional_replenish' && state.phase !== 'mandatory')
                return state;
            if ((0, helpers_1.totalTokens)(state.bag) === 0)
                return state;
            let newState = replenishBoard(state);
            // Opponent gets 1 privilege as penalty
            const opp = (1 - cp);
            const { privileges, players } = (0, helpers_1.grantPrivileges)(newState, opp, 1);
        ...
        case 'TAKE_TOKENS': {
            if (state.phase !== 'mandatory')
                return state;
            const { indices } = action;
            if (!(0, board_1.isValidTokenLine)(indices))
                return state;
            const board = [...state.board];
            let playerTokens = { ...player.tokens };
            const taken = [];
            ...
            const allSame = taken.length === helpers_1.PENALTY_SAME_COLOR_COUNT && taken.every(c => c === taken[0]);
            const twoPearls = taken.filter(c => c === 'pearl').length >= helpers_1.PENALTY_PEARL_COUNT;
        ...
        case 'RESERVE_CARD': {
            ...
            if (goldIdx === -1)
                return state;
            const sourceToLevel = { deck_1: 1, deck_2: 2, deck_3: 3 };
        ...
        case 'PURCHASE_CARD': {
            ...
            if (!(0, helpers_1.canAfford)(card, player, goldUsage))
                return state;
            ...
            const milestoneCrossed = helpers_1.CROWN_MILESTONES.some(m => prevCrowns < m && newState.players[cp].crowns >= m)
        ...
        case 'CHOOSE_ROYAL_CARD': {
            ...
            if (newState.phase === 'resolve_ability')
                return newState;
            return endOfTurnSequence(newState);
        }
        // ── Ability: Assign Wild card color ──────────────────────────────────────
        case 'ASSIGN_WILD_COLOR': {
            if (state.phase !== 'assign_wild')
                return state;
            const { wildCardId, color } = action;
            const wildCard = player.purchasedCards.find(c => c.id === wildCardId);
            if (!wildCard || wildCard.color !== 'wild')
                return state;
            // Validate: player owns at least one Jewel Card with this color
            const hasColor = player.purchasedCards.some(c => c.id !== wildCardId && c.color !== 'wild' && c.color !== null && c.color === color);
            if (!hasColor)
                return state;
            const updatedCards = player.purchasedCards.map(c => c.id === wildCardId ? { ...c, assignedColor: color } : c);
            ...
            if (newState.pendingCrownCheck)
                return { ...newState, phase: 'choose_royal', pendingCrownCheck: false };
            return endOfTurnSequence(newState);
        }
        // ── Ability: Token — take 1 token of card's color from board ─────────────
        case 'TAKE_TOKEN_FROM_BOARD': {
            if (state.phase !== 'resolve_ability')
                return state;
            const { index } = action;
        ...
        case 'DISCARD_TOKENS': {
            ...
            let playerTokens = { ...player.tokens };
            let bag = { ...state.bag };
            for (const [colorStr, amount] of Object.entries(action.tokens)) {
                if (playerTokens[colorStr] < amount)
                    return state;
                playerTokens = { ...playerTokens, [colorStr]: playerTokens[colorStr] - amount };
                bag = { ...bag, [colorStr]: bag[colorStr] + amount };
            }
            if ((0, helpers_1.totalTokens)(playerTokens) > helpers_1.MAX_TOKENS)
                return state; // still too many
            let newState = updatePlayer(state, cp, { tokens: playerTokens });
