...
exports.SPIRAL_ORDER = void 0;
exports.indexToCoord = indexToCoord;
exports.coordToIndex = coordToIndex;
exports.adjacentCells = adjacentCells;
exports.isValidTokenLine = isValidTokenLine;
...
function indexToCoord(index) {
    return [Math.floor(index / helpers_1.BOARD_WIDTH), index % helpers_1.BOARD_WIDTH];
}
function coordToIndex(row, col) {
    return row * helpers_1.BOARD_WIDTH + col;
}
// ─── Adjacency ────────────────────────────────────────────────────────────────
...
function adjacentCells(index) {
    ...
    for (let dr = -1; dr <= 1; dr++) {
        for (let dc = -1; dc <= 1; dc++) {
            ...
            const r = row + dr;
            const c = col + dc;
            if (r >= 0 && r < helpers_1.BOARD_HEIGHT && c >= 0 && c < helpers_1.BOARD_WIDTH) {
 ...
 * with no gaps.
 */
function isValidTokenLine(indices) {
    if (indices.length < 1 || indices.length > helpers_1.MAX_TOKENS_IN_LINE)
        return false;
    if (indices.length === 1)
        return true;
    ...
    for (let i = 2; i < coords.length; i++) {
        if (coords[i][0] - coords[i - 1][0] !== dr)
            return false;
        if (coords[i][1] - coords[i - 1][1] !== dc)
            return false;
    }
    return true;
}
