...
Per-player features (108):
  ...
  [99..107] royal cards: 9 floats (3 royals × 3 scalar features: points/6, crowns/3, prestige/20)
"""
...
def _encode_card(card: dict | None, out: np.ndarray, offset: int) -> None:
    ...
    out[offset + 18] = card.get("crowns", 0) / 3.0

    cost = card.get("cost", {})
    for ci, color_name in enumerate(TOKEN_COLORS):
        out[offset + 19 + ci] = cost.get(color_name, 0) / 8.0
...
def _encode_player(player: dict, out: np.ndarray, offset: int) -> None:
    ...
    for ci, color in enumerate(TOKEN_COLORS):
        out[offset + ci] = tokens.get(color, 0) / 10.0
    ...
    for ci, color in enumerate(GEM_COLORS):
        out[offset + 7 + ci] = bonuses[color] / 5.0
        out[offset + 12 + ci] = prestige_by_color[color] / 10.0

    out[offset + 17] = player.get("prestige", 0) / 20.0
    out[offset + 18] = player.get("crowns", 0) / 10.0
    out[offset + 19] = player.get("privileges", 0) / 3.0

    reserved = player.get("reservedCards", [])
    out[offset + 20] = len(reserved) / 3.0
    for i in range(3):
        card = reserved[i] if i < len(reserved) else None
        _encode_card(card, out, offset + 21 + i * CARD_FEATURES)

    # Royal cards: encode first 3 as compact scalar triples (points, crowns, prestige)
...
def encode(state: dict) -> np.ndarray:
    ...
    bag = state.get("bag", {})
    for ci, color in enumerate(TOKEN_COLORS):
        out[200 + ci] = bag.get(color, 0) / 10.0
    ...
    out[519] = len(decks.get("level1", [])) / 30.0
    out[520] = len(decks.get("level2", [])) / 30.0
    out[521] = len(decks.get("level3", [])) / 30.0
    ...
    # Table privileges
    out[626] = state.get("privileges", 0) / 3.0
    ...
    out[850] = state.get("extraTurns", 0) / 3.0
